...
/world/Topic(T, addr, master, key)
	// This is an insecure way to print this.
	...
	if (T == "status")
		...
		diary << "TOPIC: \"status\" from:[addr][log_end]"
		...
		for (var/client/C)
			x++
		diary << "TOPIC: \"ping\" from:[addr][log_end]"
		return x
		...
		for(var/mob/M in player_list)
			if(M.client)
				n++
		diary << "TOPIC: \"players\" from:[addr][log_end]"
		return n
		...
		*/
		...
		if(!config.comms_password || (input["key"] != config.comms_password))
		...
		diary << "TOPIC: \"adminmsg\" from:[addr][log_end]"
		...
		*/
		...
		diary << "TOPIC: \"notes\" from:[addr][log_end]"
