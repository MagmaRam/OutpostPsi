...
datum/directive/sexism/get_description()
	return {"
		<p>
...
datum/directive/sexism/initialize()
...
datum/directive/sexism/meets_prerequisites()
	var/females = 0
	var/males = 0
	for(var/mob/M in player_list)
...
datum/directive/sexism/directives_complete()
	return command_targets.len == count_heads_reassigned() && alien_targets.len == 0

datum/directive/sexism/get_remaining_orders()
	var/text = ""
...
/hook/reassign_employee/proc/command_reassignments(obj/item/weapon/card/id/id_card)
	var/datum/directive/sexism/D = get_directive("sexism")
	if(!D) return 1
